...
def proceso(enviroment, InstLL, CMemory, Memoria, CPU, WTime, CRandom):
    ...
    with Memoria.get(CMemory) as MemoUS:     
        yield MemoUS
           ...
           # tiempo de espera, se incia proceso y se pide RAM   
	...
	# Tiempo en ejecucion del simulador		
        SimuladorT = SimuladorT + ProcTime
        SimuladorDEV = ((SimuladorT*SimuladorT)/CREATE_PROCESS) 
        print('P: %7.4fs Memoria utilizada / Revision RAM%6.3f >> tiempo  %s' % (enviroment.now, Memoria.level, SimuladorT))        
